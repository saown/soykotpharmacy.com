...
<head>
    ...
    <link rel="stylesheet" href="<?= site_url('assets/front-end/invoice/css/style.css')?>?t=<?= time()?>">
...
<div class="container mt-4" style="overflow: hidden">
    ...
    <table style="width: 100%">
        ...
        <?php foreach ($productList as $product):?>
            <tr>
                ...
                <td><?= number_format($product->single_price * $product->quantity,2)?> tk</td>
            </tr>
        <?php endforeach;?>
        </tbody>
    ...
    <div class="float-start mt-5">
        <a href="<?= site_url('pdfDownload/').$orderInfo->id?>" class="btn btn-success">Download Invoice</a>
    </div>
    <div class="float-end" style="width: 220px; height: auto; line-height: 20px; margin-top: 25px">
        ...
        <div class="text-end">Delivery Fee : <?= $deliveryFree?> tk</div>
        <div class="text-end">Grand Total : <?= $grandTotal?> tk</div>
    </div>
</div>

<div class="text-center mt-5 mb-5" style="overflow: hidden; cursor: pointer"><button class="btn btn-primary btn-sm go-back"><-Back</button></div>
    <script>
    ...
    <script src="<?= site_url('assets/sweetalert2/sweetalert2.js') ?>?t=<?= time()?>"></script>
    <script src="<?= site_url('assets/front-end/invoice/js/invoice.js') ?>?t=<?= time()?>"></script>
</body>
</html>